...
function createMoneySpentByVendor() {
    $.ajax({
        type: 'POST',
...
function updatedVendorPop(vendor, values) {
  var pie = document.getElementById("MostPopVendors");
  pie.labels = vendor;
  pie.data = values;

function updatedMacro(values) {
  var pie = document.getElementById("Marconutrients");
  pie.labels = ["Carbs", "Protein", "Fat"];
  pie.data = values.slice(1, values.length);
}
function updatedVendorCals(vendor, values) {
  var pie = document.getElementById("CalCountVendor");
  pie.labels = vendor;
  pie.data = values;
...
// STATISTIC PAGE CHARTS
// -- Most popular vendors (number of times visited)
...
// -- Most popular food items purchased
